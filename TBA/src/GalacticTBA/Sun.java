...
public class Sun extends Planet {
	...
	public Sun(Vector3f axis, float radius, float orbit_radius, Color3f color) {
		...
		// this.setColor(color);
		this.radius = radius / PLANET_RADIUS_DIVISOR;
		this.axis = axis;
		this.orbit_radius = orbit_radius / PLANET_SIZE_DIVISOR;
		...
		// light.setPosition(new
		// Point3f(position.getX(),position.getY(),position.getZ()));
		...
		this.tg_trans_light = translate(this.light, new Vector3f(
				this.orbit_radius, 0, 0));
		this.tg_rot_light = rotate(this.tg_trans_light, new Alpha(-1,
				(long) ((radius + orbit_radius) * ORBIT_SPEED_MODIFIER)));
